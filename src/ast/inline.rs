...
pub fn inline(pat: Expression, args: HashMap<String, Expression>) -> Expression {
  match pat {
    ...
    Expression::Record(items) => Expression::Record(
      items
        .into_iter()
        .map(
          |expr_struct::RecordItem { key, value }| expr_struct::RecordItem {
            key:   match key {
