...
impl Display for Expression {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      Self::BinaryExpression(Some(left), op, Some(right)) => write!(f, "({:?} {} {})", op, left, right),
...
impl Expression {
  fn parse2(
    ...
    loop {
      ...
      //   token_stream.next();
      //   lhs = Expression::BinaryExpression(Some(Box::new(lhs)), Some(token), None);
      //   continue;
      // }
...
fn expr_bp(lexer: &mut TokenStream) -> Option<Expression> {
  ...
  loop {
    ...
    let (token, r_bp) = loop {
      match binding_power(token.clone(), top.lhs.is_none()) {
        ...
        _ => {
          ...
          // top.lhs = Some(Expression::Cons(token, args));
          top.lhs = Some(Expression::BinaryExpression(
            top.lhs.map(Box::new),
            token.clone().map(|token| token.token),
            res.lhs.map(Box::new),
          ))
        },
      };
    ...
    if token.token == Token::RParenthesis {
      // assert_eq!(top.token, Some(Token::RParenthesis));
      let res = top;
